      ...
      {selectedMatch.detailedStats?.map((stat: DetailedStat, index: number) => {
        ...
        return (
          <div key={index} className="flex justify-between items-center">
            <span className={`w-16 text-right font-bold text-foreground ${stat.label.includes('xG') ? 'text-lg' : 'text-xl'}`}>{stat.homeDisplay}</span>
            <span className="flex-1 mx-4">
                <div className="text-center text-sm text-secondary mb-1 font-semibold">{stat.label}</div>
            ...
            <span className={`w-16 font-bold text-foreground ${stat.label.includes('xG') ? 'text-lg' : 'text-xl'}`}>{stat.awayDisplay}</span>
          </div>
        );
      })}
    </div>
